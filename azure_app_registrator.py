...
def create_application(headers):
    ...
    response = requests.post(f"{GRAPH_API}/applications", headers=headers, json=app_data)

    if response.status_code == 201:
        app_info = response.json()
        return app_info["appId"], app_info["id"], app_info["displayName"]
    else:
        print(f"{RED}Error creating application: {response.status_code}, {response.text}{ENDC}")
        exit(1)
...
def grant_permissions(headers, object_id):
    ...
    required_access = [
        {"resourceAppId": app_id, "resourceAccess": permissions} for app_id, permissions in PERMISSIONS.items()
    ]
    ...
    if response.status_code == 204:
        ...
        print(f"{RED}Error assigning permissions: {response.status_code}, {response.text}{ENDC}")
        exit(1)
...
def grant_admin_consent_uri(app_id, tenant_id):
    ...
    consent_request_url = f"https://login.microsoftonline.com/{tenant_id}/adminconsent?client_id={app_id}"
...
def generate_client_secret(headers, object_id):
    ...
    if response.status_code == 200:
        ...
        print(f"{MAGENTA}Note this client secret for future use, it will not be shown again.{ENDC}")
        print(f"{MAGENTA}Client Secret: {client_secret}{ENDC}")
        return client_secret
    else:
        print(f"{RED}Error generating client secret: {response.status_code}, {response.text}{ENDC}")
        exit(1)
...
def main():
    ...
    try:
        ...
        print("Authentication failed. Please ensure the account belongs to the specified tenant.")
        sys.exit(1)
